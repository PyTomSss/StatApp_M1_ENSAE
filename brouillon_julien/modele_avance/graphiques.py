...
for nb_assets in range(5, 400):
    # Générer les matrices in-sample et out-sample avec le nombre d'actifs actuel
    matrices_in_sample, matrices_out_sample = retourner_matrices(nb_signals=100, nb_assets=nb_assets)
    ...
    rendements_par_jour_is = np.mean(rendements_par_jour_par_actifs_is, axis=1)
    
    # Calculer le rendement moyen et la volatilité du portefeuille pour les données in-sample
    rendement_moyen_is = np.mean(rendements_par_jour_is)
    volatilite_portefeuille_is = np.std(rendements_par_jour_is)
    ...
    rendements_par_jour_os = np.mean(rendements_par_jour_par_actifs_os, axis=1)
    
    # Calculer le rendement moyen et la volatilité du portefeuille pour les données out-sample
    rendement_moyen_os = np.mean(rendements_par_jour_os)
    volatilite_portefeuille_os = np.std(rendements_par_jour_os)
...
plt.plot(range(5, 400), sharpes_ratios_is, label='In-sample')

# Plot des données out-sample
plt.plot(range(5, 400), sharpes_ratios_os, label='Out-sample')
