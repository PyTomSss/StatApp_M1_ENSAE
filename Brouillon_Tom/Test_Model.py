def test_model(X_train, X_test, y_train, y_test, DF, model) : 
    ...
    reg.fit(y_test.reshape(-1, 1), y_pred.reshape(-1, 1))
    ...
    plt.plot(y_test.reshape(-1, 1), reg.predict(y_test.reshape(-1, 1)), color='red',linewidth = 3, label='Régression Linéaire')
